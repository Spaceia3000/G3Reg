...
namespace pcl {
    ...
    template<typename T>
    void issKeyPointExtration(pcl::shared_ptr<pcl::PointCloud<T>> cloud, pcl::shared_ptr<pcl::PointCloud<T>> ISS,
    ...
    template<typename T>
    void voxelize(
            const pcl::shared_ptr<pcl::PointCloud<T>> srcPtr, pcl::shared_ptr<pcl::PointCloud<T>> dstPtr,
            double voxelSize) {
