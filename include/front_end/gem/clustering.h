...
namespace DCVC {
    ...
    void Cluster(pcl::shared_ptr<pcl::PointCloud<PointT>> cloud,
                 std::vector<g3reg::ClusterFeature::Ptr> &clusters,
                 bool remove_ground = true) {
        double tSrc;
        pcl::PointCloud<PointT> srcGround;
        pcl::shared_ptr<pcl::PointCloud<PointT>> ptrSrcNonground(new pcl::PointCloud<PointT>);
        if (remove_ground) {
        ...
        std::vector<pcl::shared_ptr<pcl::PointCloud<PointT>>> clusters_pcl;
...
namespace travel {
    template<typename PointT>
    void
    Cluster(pcl::shared_ptr<pcl::PointCloud<PointT>> cloud_ptr, std::vector<g3reg::ClusterFeature::Ptr> &clusters,
            bool remove_ground = true) {
        double tSrc;
        pcl::PointCloud<PointT> srcGround;
        pcl::shared_ptr<pcl::PointCloud<PointT>> ptrSrcNonground(new pcl::PointCloud<PointT>);
...
namespace pcl {
    ...
    void Cluster(pcl::shared_ptr<pcl::PointCloud<PointT>> cloud,
        ...
        for (int i = 0; i < cluster_res.size(); ++i) {
            pcl::shared_ptr<pcl::PointCloud<PointT>> plane_ins(new pcl::PointCloud<PointT>());
